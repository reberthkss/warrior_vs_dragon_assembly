...
O combate é baseado em turnos. O jogador tem acesso a cinco ações distintas:
\begin{enumerate}
    \item \textbf{Ataque Normal}: Dano padrão com uma taxa de acerto equilibrada (80\%).
    \item \textbf{Espada (Sword)}: Um movimento tático que atordoa o dragão, fazendo-o perder um turno, mas não causa dano direto.
    \item \textbf{Flanco (Flank)}: Um ataque de alto risco e alta recompensa com 40\% de chance de acerto crítico.
    \item \textbf{Lança (Lance)}: Uma postura defensiva que causa menos dano, mas aumenta a evasão do jogador para o próximo turno.
    \item \textbf{Quiz}: Uma habilidade especial que aciona uma pergunta sobre arquitetura de computadores.
\end{enumerate}

O Dragão atua como o oponente de IA com três comportamentos aleatórios:
\begin{itemize}
    \item \textbf{Sopro de Fogo}: Ataque de alto dano.
    \item \textbf{Pisar (Stomp)}: Atordoa o jogador.
    \item \textbf{Voar (Fly)}: Aumenta a evasão do dragão, tornando-o mais difícil de acertar.
\end{itemize}
